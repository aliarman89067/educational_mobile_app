...
const FriendResult = () => {
  const { friendResultId: resultId, roomId } = useLocalSearchParams();
  ...
  useEffect(() => {
    ...
    socketIo.emit("get-friend-history", { resultId, roomId });
    ...
    socketIo.on("get-friend-history-data", handleHistoryData);
    socketIo.on("get-friend-history-error", handleHistoryError);

    return () => {
      socketIo.off("get-friend-history-data", handleHistoryData);
      socketIo.off("get-friend-history-error", handleHistoryError);
    };
  ...
  const loadData = async () => {
    try {
      ...
      const { data } = await axios.get(
        `/quiz/get-friend-history/${resultId}/${roomId}`
      );
      if (data.success) {
        if (data.isPending) {
          setIsPending(true);
